...
bool Graphics::init(Input* manager) {
	...
	window = std::make_unique<Window>(SCR_DIM.x, SCR_DIM.y);
	if (!window->init()) {
	...
	input_manager->windowSizeInput(SCR_DIM.x, SCR_DIM.y);

	shader_program = new ShaderProgram("shaders/test.vert", "shaders/test.frag");

	fire_tex = new Texture("textures/fire.png", GL_NEAREST);
	diamond_tex = new Texture("textures/diamond.png", GL_NEAREST);
	player_ship_tex = new Texture("textures/ship.png", GL_NEAREST);
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(glm::mat4) * n_sprites, transforms.data(), GL_DYNAMIC_DRAW);
	glBindBuffer(GL_ARRAY_BUFFER, 0);

	// Texture id buffer object
	glGenBuffers(1, &IBO);
	glBindBuffer(GL_ARRAY_BUFFER, IBO);
	glBufferData(GL_ARRAY_BUFFER, sizeof(glm::int32) * n_sprites, texture_ids.data(), GL_DYNAMIC_DRAW);
	glBindBuffer(GL_ARRAY_BUFFER, 0);
...
void Graphics::updateSprites()
{
	...
	for (int i = 0; i < n_sprites; i++)
	{
		...
		transform_mat = glm::rotate(transform_mat, sprite->rotation.getValue(), glm::vec3(0.0f, 0.0f, 1.0f));
